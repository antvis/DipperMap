...
export default function DatasetList({ className }: IProps) {
  const { datasetList, setDatasetList, selectDatasetId, setSelectDatasetId } = useModel('dataset');
  const {
    layerList,
    filterList,
    interactiveList,
  ...
  const { onDragEnd, onEditName, onDelete } = useListHook(datasetList, setDatasetList);
            ...
            <div className={styles.appDatasetItemContent}>
              <div className={styles.btnGroup}>
                <Tooltip overlay="复制" placement="bottom">
                  <i className="dpiconfont dpicon-fuzhi is-link" onClick={() => onCopy(dataset)} />
                </Tooltip>
                <Popconfirm title="确认删除此数据源" onConfirm={() => checkDelete(dataset)}>
