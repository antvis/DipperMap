...
const AppFilterConfig = () => {
  const { filterList, setFilterList, filterByDatasetId } = useModel('config');
  const { selectDataset } = useModel('dataset');
  const { addFilter, copyFilter } = useFilter();
  const { onEditName, onDragEnd, onDelete, onChange } = useListHook(filterList, setFilterList);
    ...
    <div className={styles.filterList}>
      <div className={styles.filterListContent}>
        {!displayFilterList.length ? (
          ...
          <DragList itemClassName={styles.filterItem} items={displayFilterList} onDrag={onDragEnd}>
            {(filter, icon) => (
              <FilterItem
                filter={filter}
                dragIcon={icon}
