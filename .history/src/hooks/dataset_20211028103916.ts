...
const useDataset = () => {
  const { datasetList, setDatasetList } = useModel('dataset');

  const getNewDatasetName = useCallback(() => {
    return generateUnRepeatValue<IDataset, string>(datasetList, 'name', '数据源');
  }, [datasetList]);

  const addDataset: (params: Partial<IDataset> & { data: any[] }) => Promise<IDataset> =
    useCallback(
  ...
  const getNewDatasetFields: (data: any[]) => IDatasetField[] = useCallback((data) => {
    if (!data.length) {
      return [];
    }
    ...
    if (Array.isArray(content)) {
      ...
      try {
        if (content.startsWith('[') && content.endsWith(']')) {
          ...
          result = papaparse.parse(content, { header: true, skipEmptyLines: true }).data ?? [];
          isCSV = true;
        }
      } catch (e) {
